...
export default function ConverterClient({ user }: ConverterClientProps): React.JSX.Element {
  ...
  const handleFolderUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const uploadPromises = Array.from(selectedFiles).map(async (file) => {
      ...
      if (!response.ok) { return { error: await response.text(), file: file.name }; }
      return response.ok;
    });
  ...
  const handleZipUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      showAlert({ title: 'Processing Failed', message: error instanceof Error ? error.message : 'Failed', variant: 'destructive' });
    } finally {
  ...
  return (
    <TooltipProvider>
      <main className="h-dvh w-full bg-slate-950 text-slate-100 flex flex-col overflow-hidden relative selection:bg-blue-500/30">
        ...
        <input type="file" ref={folderInputRef} onChange={handleFolderUpload} className="hidden" {...( { webkitdirectory: "", directory: "" } as React.InputHTMLAttributes<HTMLInputElement> )} />
        <input type="file" multiple accept=".zip,.7z,.rar,.tar,.tar.gz,.tgz" ref={zipInputRef} onChange={handleZipUpload} className="hidden" />
                ...
                {!loading && filteredMdFiles.length > 0 ? (
                  filteredMdFiles.map((file, index) => (
                    <FileRow
                      ...
                      isSelectionMode={isSelectionMode}
                      processingState={processingStates[file.id] || (file.metadata?.generatedPdfUrl ? 'done' : 'pending')}
