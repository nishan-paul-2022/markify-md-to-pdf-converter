...
- **Hooks**: kebab-case (`use-files.ts`, `use-converter.ts`) -> _Standard_
- **Components**: PascalCase (`EditorView.tsx`, `PdfViewer.tsx`) -> _Standard_
...
_Overall Review: The project has a very high-quality visual finish and a solid technical foundation. Addressing the architectural storage and state management concerns would move this from a "Great Project" to a "World-Class Application."_
